...
const CRYPTO_LIST = [
    ...
    { id: 'cardano', name: 'ADA', symbol: 'ADA' },
    { id: 'dogecoin', name: 'DOGE', symbol: 'DOGE' },
    { id: 'polkadot', name: 'DOT', symbol: 'DOT' },
...
function renderChart(coin, processedData, days) {
    ...
    chartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { grid: { display: false }, ticks: { color: getComputedStyle(document.body).getPropertyValue('--text-color'), maxTicksLimit: days === 7 ? 7 : days === 30 ? 10 : 12, font: { size: 10 } } },
                y: { beginAtZero: false, grid: { color: 'rgba(255, 255, 255, 0.1)', borderDash: [5,5] }, ticks: { color: getComputedStyle(document.body).getPropertyValue('--text-color'), callback: v => '$' + v.toLocaleString('pt-BR', {minimumFractionDigits:0,maximumFractionDigits:0}), font: { size: 10 } } }
            }
        }
    });
}
...
// ================= Compartilhamento =================
function shareOnFacebook(url,title){window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}&quote=${encodeURIComponent(title)}`,'_blank');}
function shareOnInstagram(url,title){alert('Compartilhe no Instagram manualmente!');}
function shareOnTwitter(url,title){window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(title)}`,'_blank');}
function shareOnWhatsApp(url,title){window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(title+' - '+url)}`,'_blank');}
function shareByEmail(url,title){window.location.href = `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(url)}`;}
function copyLink(url){navigator.clipboard.writeText(url).then(()=>{alert('Link copiado para a área de transferência!');});}
...
chartSelect.addEventListener('change', (e)=>{fetchChartData(e.target.value,currentChartPeriod);});
